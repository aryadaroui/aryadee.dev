<script lang="ts">
	...
	const x_scale_graph = d3
		.scaleLinear()
		.domain([0, 100])
		.range([0, width_graph]);
	const y_scale_graph = d3
		.scaleLinear()
		.domain([0, 100])
		.range([height_graph, 0]);

	const y_scale_inv_graph = d3
		.scaleLinear()
		.domain([height_graph, 0])
		.range([0, 100]);

	const x_scale_inv_graph = d3
		.scaleLinear()
		.domain([width_graph, 0])
		.range([100, 0]);
	...
	const x_scale_slider = d3
		.scaleLinear()
		.domain([0, 100])
		.range([0, width_slider]);

	const y_scale_slider = d3
		.scaleLinear()
		.domain([-100, 100])
		.range([height_slider, 0]);

	const y_scale_inv_slider = d3
		.scaleLinear()
		.domain([height_slider, 0])
		.range([-100, 100]);
	...
	onMount(() => {
		const graph = d3
			...
			.attr(
				"height",
				height_graph + margin_graph.top + margin_graph.bottom
			)
			.append("g")
			.attr(
				"transform",
		...
		const slider = d3
			...
			.attr(
				"width",
				width_slider + margin_slider.left + margin_slider.right
			)
			.attr(
				"height",
				height_slider + margin_slider.top + margin_slider.bottom
			)
			.append("g")
			.attr(
				"transform",
				`translate(${margin_slider.left}, ${margin_slider.top})`
			);
